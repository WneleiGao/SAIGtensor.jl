# ==============================================================================
#                 test tensor and cp tensor type
# ==============================================================================
using SAIGtensor
...
N = 3; I = [111, 121, 321]; D = reshape(collect(1.0:Float64(prod(I))), I...);
X = initTensor(N, I, D)
tmp = 0.
for n = 1 : N
...
# recursive Khatri-Rao product for matrix
I = [21, 32, 43, 24]; R = 3;
N = length(I);
A = Array{Array{Float64,2}}(N)
for m = 1 : N
...
# test on tensor times a set of vectors, the order can be changed
I = [77,34,89,67];
...
# better understanding the properties of permutedims
...
# ==============================================================================
#                 the properties of tensor times vector
# ==============================================================================
# test left side
I = [73,27,33,17]; N = length(I);
X = rand(I...); X1 = copy(X);
...
# ===========test tensor times a set of vectors=================================
I = [33,27,35,41,29]; N = length(I);
X = tensor(N, I, rand(I...));
dims = [5, 2, 4]
...
# =======testing sampled KRP for 3D tensor===================
N = 3; I = [11,12,13]; R = 20;
ns = ceil(Int64, 10*R*log(R))
lambda = randn(R)
A = Array{Array{Float64,2}}(N);
...
Zs = zeros(ns, R);
n = 2;
idx = randSpl!(Zs, Xst, A, T, ns, n);

It = (setdiff(I,[I[n]])...)
rind = zeros(Int64, ns)
for i = 1 : ns
    rind[i] = sub2ind(It, idx[1][i], idx[2][i])
end
...
P = cptensor(N, I, R, lambda, A);
X = cp2tensor(P);
(lambda1, A1) = randCpAls(X, R, maxiter=50, fitchangetol=1e-6, pflag=true);
(lambda2, A2) = cpals(X, R, maxiter=50, fitchangetol=1e-6, pflag=true);
